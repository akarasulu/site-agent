...
def build_config_coverage_report(
    ...
    field_elements = [element for element in snapshot.elements if element.control_type in CONFIG_FIELD_TYPES]
    ...
    action_tool_evidence = {
        evidence_id
        for tool in tools
        if tool_value(tool, "source_type") in {"ui_form", "ui_flow"}
    ...
    for form in snapshot.forms:
        ...
        configurable = [element for element in form_fields if element.control_type in CONFIG_FIELD_TYPES and element.control_type != "hidden"]
        ...
        if configurable and not (field_evidence & action_tool_evidence):
    ...
    if previous_snapshot:
        previous_field_keys = {f"{element.page_id}|{element.label}|{element.control_type}" for element in previous_snapshot.elements if element.control_type in CONFIG_FIELD_TYPES}
        ...
        previous_counts = {
            ...
            "fields": sum(1 for element in previous_snapshot.elements if element.control_type in CONFIG_FIELD_TYPES),
    ...
    action_tool_ratio = _ratio(len(snapshot.forms) - len(forms_without_action_tools), len(snapshot.forms))
